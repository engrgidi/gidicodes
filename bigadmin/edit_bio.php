...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE  admin_profile SET bio='$bio', image='$filename' WHERE id='$id' ";
    $result = $conn->query($sql);
    if ($result === TRUE) {
        if (move_uploaded_file($tempname, $folder)) {
            $msg = "<div class='alert alert-success'>Bio Updated successfully</div>";
        } else {
                        ...
                        ?>
                                <form id="contact-form" enctype="multipart/form-data" role="form" action="<?php $_SERVER['PHP_SELF']; ?>" class="contact-form" method="post">
                                    <div class="row">
                                        ...
                                        <div class="col-xl-6 col-lg-6 mb-20">
                                            <textarea name="bio" cols="30" rows="10" placeholder="About Us*" required><?php print $row['bio']; ?></textarea>
                                        </div>
                                <?php }
                        } ?>
